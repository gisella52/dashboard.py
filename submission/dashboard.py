...
with st.sidebar:
    ...
    # Ubah 0 jadi tahun 2011 dan 1 jadi tahun 2012
    year_mapping = {0: 2011, 1: 2012}
    year_list = list(day_df.yr.map(year_mapping).unique())[::-1]
    
    selected_year = st.selectbox('Select a year', year_list, index=len(year_list)-1)
    
    # Ubah lagi tahun 2011 jadi 0 dan tahun 2012 jadi 1
...
# main page
st.title(f'Analisis Bike Sharing Di Tahun {selected_year}')
...
tab1, tab2 = st.tabs(["Halaman 1", "Halaman 2"])
 
   

with tab1:
    st.header(f'Jumlah Registered Users dan Casual Users - Tahun {selected_year}')
    plt.figure(figsize=(8, 6))
    sns.barplot(x='total_registered', y='total_casual', data=df_selected_year)
    plt.title(f'Jumlah Registered Users dan Casual Users - Tahun {selected_year}')
